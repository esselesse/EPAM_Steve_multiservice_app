...
public class WHEREParser {
    ...
    public boolean evaluate(Map<String, String> values, String whereString) {
        ...
        if (!validate(whereString)) {
            throw new RuntimeException("INVALID CLAUSE: "  + whereString);//todo сделать не рантайм
        }

        //extract clause - вытаскиваем строку после where
        String clause = extractClause(whereString);
        clause = beautify(clause);//убираем пробелы и прочую хрень
        if (clause == null) {
        ...
        for (String ORStatement : ORStatements) {//для каждой разбитой по ору строчки
            String[] ANDStatements = ORStatement.split(AND);//разбиваем ее по энду
            Boolean[] ANDResults = new Boolean[ANDStatements.length];
            int andIndex = 0;
            for (String statement : ANDStatements) {//вычисляем
    ...
    public boolean evaluateStatement(String statement){//вытягиваем конкретные значения
        ...
        BooleanOperator operator = BooleanOperator.get(operatorSymbol);//todo проверка - есть ли в мапе этот оператор
