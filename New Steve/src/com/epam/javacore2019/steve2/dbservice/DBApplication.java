...
public enum DBApplication {
    ...
    public void start(int port) {
        ...
        if (testEnabled) {
            try {
                runTests("com.epam.javacore2019.steve2.dbservice.test.WHERETest");//где все тесты лежат
            }catch (Exception e) {
                e.printStackTrace();
            }
        }
    ...
    private void runTests(String className) throws Exception {
        ...
        for (Method m : Class.forName(className).getMethods()) {//рефлексия! достаем все методы
            Test testAnnotation = m.getAnnotation(Test.class);
            if (testAnnotation != null && testAnnotation.enabled()) {//если есть аннотация @test, врубаем тест
                try {
                    m.invoke(null);
